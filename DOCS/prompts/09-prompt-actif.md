...
```bash
docker compose up -d grafana sqlite
docker compose run --rm collector
docker compose logs --tail=200 collector

docker compose exec sqlite sqlite3 -header -column /data/macro.db "
SELECT source, symbol, value, unit, observed_at
FROM macro_series
WHERE symbol IN (
  'VIX', 'USD_BROAD', 'US2Y', 'US10Y', 'WTI', 'BRENT',
  'BTC_DOM', 'STABLECOIN_DOM', 'ETH_BTC', 'TOTAL3',
  'MARKET_REGIME_SCORE', 'MARKET_REGIME_STATE'
)
ORDER BY observed_at DESC, id DESC;
"

python3 -m json.tool grafana/dashboards/crypto-macro-overview.json >/dev/null
```

Adapter l'exécution au contexte : Docker, `.env` et accès réseau aux APIs sont
nécessaires à une collecte réelle.
